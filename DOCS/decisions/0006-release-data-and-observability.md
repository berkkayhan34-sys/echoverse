...
- Current server-owned moderation records, reports, and deleted-message
  tombstones use a 180-day retention window. The scheduled cleanup is
  idempotent, removes only expired records, and does not replace the separate
  account, backup, export, or host-log deletion procedures.
