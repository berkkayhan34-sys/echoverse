...
.PHONY: help version-check roadmap-check docs-check metadata-check ai-check ai-test \
	...
	install-deps tools-install setup tooling-check format-check format lint reuse-check secret-scan \
...
metadata-check: version-check
	@node -e "const fs=require('fs'); for(const p of ['package.json','project/apps/server/package.json','project/apps/web/package.json','project/apps/desktop/package.json','project/packages/contracts/package.json','project/packages/config/package.json','project/packages/client-core/package.json','project/packages/shared-ui/package.json']) JSON.parse(fs.readFileSync(p,'utf8')); console.log('Package metadata verified')"
...
quality: ai-check tooling-check format-check lint reuse-check secret-scan localization-check dependency-check typecheck test coverage
...
work-init:
	@node -e "const fs=require('fs'); for(const p of ['tmp','tmp/coverage','tmp/test-results','tmp/runtime','tmp/environments','tmp/generated']) fs.mkdirSync(p,{recursive:true}); console.log('Created ignored tmp directories')"
